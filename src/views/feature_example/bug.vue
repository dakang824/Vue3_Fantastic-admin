<template>
    <div>
        <page-header title="错误日志" content="错误日志通过 Vue 提供的全局错误钩子 errorHandler 进行拦截，如果需要上报给后端，需自行实现">
            <el-button icon="el-icon-link" @click="open('https://v3.cn.vuejs.org/api/application-config.html#errorhandler')">Vue errorHandler 说明</el-button>
        </page-header>
        <page-main>
...
export default {
    ...
    computed: {
        errorLogInfo() {
            return this.errorLog ? [JSON.parse(this.errorLog)] : []
        }
    },
    methods: {
        bugTest(type) {
