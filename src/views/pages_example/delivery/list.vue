                    ...
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="模版名称">
                                <el-input v-model="data.search.title" placeholder="请输入模版名称，支持模糊查询" clearable @keydown.enter="currentChange()" @clear="currentChange()" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-form-item>
...
<script setup name="PagesExampleDeliveryList">
...
function getDataList() {
    ...
    proxy.$api.get('pages_example/delivery/list', {
        baseURL: '/mock/',
        params
    }).then(res => {
...
function onDel(row) {
    proxy.$confirm(`确认删除「${row.title}」吗？`, '确认信息').then(() => {
        proxy.$api.post('pages_example/delivery/delete', {
            id: row.id
        }, {
            baseURL: '/mock/'
        }).then(() => {
...
function onChangeStatus(val, row) {
    proxy.$confirm(`确认${val ? '启用' : '停用'}「${row.title}」运费模版吗？`, '确认信息').then(() => {
        proxy.$api.post('pages_example/delivery/change/status', {
            id: row.id,
            status: val
        }, {
            baseURL: '/mock/'
