...
function getInfo() {
    ...
    proxy.$api.get('pages_example/form_mode/detail', {
        baseURL: '/mock/',
        params: {
    ...
    submit(callback) {
        if (data.value.form.id == '') {
            proxy.$refs['form'].validate(valid => {
                if (valid) {
                    proxy.$api.post('pages_example/form_mode/create', data.value.form, {
                        baseURL: '/mock/'
                    }).then(() => {
            ...
            proxy.$refs['form'].validate(valid => {
                if (valid) {
                    proxy.$api.post('pages_example/form_mode/edit', data.value.form, {
                        baseURL: '/mock/'
                    }).then(() => {
                        proxy.$message.success({
                            message: '模拟编辑成功',
